...
@section('title', 'ویرایش بوت‌کمپ')

@section('content')
<div class="container-xxl flex-grow-1 container-p-y">
  <h4 class="py-3 breadcrumb-wrapper mb-4">
    <span class="text-muted fw-light">مدیریت بوت‌کمپ / بوت‌کمپ‌ها /</span> ویرایش بوت‌کمپ
  </h4>

  <div class="card">
    <div class="card-header">
      <h5 class="mb-0">ویرایش بوت‌کمپ</h5>
    </div>
    <div class="card-body">
      <form action="{{ route('admin.bootcamps.update', $bootcamp) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')

        <div class="mb-3">
          <label for="title" class="form-label">عنوان <span class="text-danger">*</span></label>
          <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $bootcamp->title) }}" required>
          @error('title')
        ...
        <div class="mb-3">
          ...
          <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="8" required>{{ old('description', $bootcamp->description) }}</textarea>
          @error('description')
          <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
        ...
        <div class="mb-3">
          ...
          <div id="icon-image-preview-container" class="mt-3">
            @if($bootcamp->getFirstMediaUrl('icon_image'))
            <img id="icon-image-preview" src="{{ $bootcamp->getFirstMediaUrl('icon_image') }}" alt="Current Icon" class="img-thumbnail" style="max-width: 300px; display: block;">
            @else
        ...
        <div class="mb-3">
          ...
          <div id="top-image-preview-container" class="mt-3">
            ...
            <img id="top-image-preview" src="{{ $bootcamp->getFirstMediaUrl('top_image') }}" alt="Current Image" class="img-thumbnail" style="max-width: 300px; display: block;">
            @else
        ...
        <div class="mb-3">
          ...
          <div class="invalid-feedback">{{ $message }}</div>
          @enderror
          <div class="form-text">حداکثر حجم هر فایل: 2 مگابایت - می‌توانید چند تصویر انتخاب کنید. تصاویر جدید جایگزین تصاویر قبلی خواهند شد.</div>

          <div class="mt-3">
            <p class="form-label">تصاویر فعلی:</p>
            <div class="d-flex flex-wrap gap-2">
              @forelse($bootcamp->getMedia('gallery_images') as $media)
              <img src="{{ $media->getUrl() }}" alt="Gallery Image" class="img-thumbnail" style="width: 150px; height: 150px; object-fit: cover;">
              @empty
              <p class="text-muted">هیچ تصویری در گالری موجود نیست</p>
              @endforelse
            </div>
          </div>
        ...
        <div class="mb-3">
          ...
          @if($bootcamp->getFirstMediaUrl('video'))
          <div class="mt-3">
        ...
        <div class="mb-3">
          ...
          <div id="video-thumbnail-preview-container" class="mt-3">
            @if($bootcamp->getFirstMediaUrl('video_thumbnail'))
            <img id="video-thumbnail-preview" src="{{ $bootcamp->getFirstMediaUrl('video_thumbnail') }}" alt="Current Video Thumbnail" class="img-thumbnail" style="max-width: 300px; display: block;">
            @else
